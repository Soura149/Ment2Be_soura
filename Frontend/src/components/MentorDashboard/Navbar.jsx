...
const MentorNavbar = ({ userName = "Mentor" }) => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 supports-backdrop-blur:bg-black/80 border-b border-[#121212] shadow-lg ${isScrolled ? "bg-black/80 backdrop-blur-lg shadow-white/20" : "bg-black shadow-white/10"}`}
      style={{
    ...
    >
      <div className="px-0">
        <div className="flex items-center justify-between h-14">
          ...
          <Link
            to="/mentor/dashboard"
            className="flex items-center gap-2 hover:opacity-80 transition-opacity pl-4"
          >
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden pb-3 space-y-1 border-t border-gray-700 bg-black/95">
            {menuItems.map((item) => {
              <div className="px-3 pt-2 border-t border-gray-700 sm:hidden">
                <UserProfileSidebar userName={userName} />
              </div>;
              const Icon = item.icon;
              return (
