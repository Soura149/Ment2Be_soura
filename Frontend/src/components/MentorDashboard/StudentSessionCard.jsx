...
export function StudentSessionCard({
  ...
  const statusConfig = {
    scheduled: {
      label: "Scheduled",
      className: "bg-[#2a2d32] text-white border-[#404040]",
    },
    "in-progress": {
      label: "In Progress",
      className: "bg-[#2a2d32] text-white border-[#404040]",
    ...
    completed: {
      label: "Completed",
      className: "bg-[#202327] text-[#535353] border-[#404040]",
    },
    cancelled: {
      label: "Cancelled",
      className: "bg-[#202327] text-[#535353] border-[#404040]",
    ...
    pending: {
      label: "Pending",
      className: "bg-[white] text-[#535353] border-[#404040]",
    },
  };
        ...
        <div className="p-0">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#404040]">
            <div className="flex items-center gap-3">
              <div className="relative h-11 w-11">
                <img
                  src={
                    studentAvatar ||
                    `https://avatar.vercel.sh/${studentEmail}.png`
                  }
                  alt={studentName}
                  className="h-full w-full rounded-full object-cover border-2 border-[#404040]"
                />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-xs text-[#535353] truncate">
                  {studentEmail}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span
                className={`text-xs font-medium px-2 py-1 rounded-md border ${currentStatus.className}`}
              >
                {currentStatus.label}
              </span>
              <Dropdown
              ...
              >
                <DropdownItem onClick={() => setIsDetailsOpen(true)}>
                  View Details
                </DropdownItem>
                <DropdownItem>Reschedule</DropdownItem>
                <DropdownItem>Send Reminder</DropdownItem>
                <DropdownItem className="text-[#b3b3b3]">
                  Cancel Session
                </DropdownItem>
              </Dropdown>
            </div>
          ...
          <div className="px-5 py-4 border-b border-[#404040]">
            <p className="text-xs font-medium text-[white] uppercase tracking-wide mb-3">
              Activity Stats
            </p>
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {[
                ...
                <div
                  key={label}
                  className="flex flex-col items-center p-2 rounded-lg bg-[#202327]"
                >
                  <Icon className={`h-5 w-5 ${iconColor} mb-1`} />
                  <span className="text-lg font-bold text-white">{value}</span>
                  <span className="text-[10px] text-[#535353] text-center leading-tight">
                    {label}
                  </span>
                </div>
              ))}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-[#535353]">
                  {sessionType === "online" ? "Online Session" : "In-Person"}
                </p>
              ...
              <>
                <button
                  onClick={() =>
                    onConfirmSession &&
                    latestSession &&
                    onConfirmSession(latestSession)
                  }
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-4 p-4 bg-[#202327] rounded-lg">
            <img
              src={
                studentAvatar || `https://avatar.vercel.sh/${studentEmail}.png`
              }
              alt={studentName}
              className="h-16 w-16 rounded-full object-cover border-2 border-[#404040]"
            />
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-sm text-[#535353] mt-1">
                <FiMail className="h-3.5 w-3.5 text-white" />
                <span>{studentEmail}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-[#535353] mt-0.5">
                <FiPhone className="h-3.5 w-3.5 text-white" />
          ...
          <div>
            ...
            <div className="space-y-2">
              {sessionHistory.map((session) => {
                const statusInfo = historyStatusConfig[session.status] || {
                  icon: FiClock,
                ...
                return (
                  <div
                    key={session.id}
                    className="flex items-center justify-between p-3 bg-[#202327] rounded-lg"
                  >
                    <div className="flex items-center gap-3">
                      <StatusIcon
                        className={`h-4 w-4 ${statusInfo.className}`}
                      />
                      <div>
                        <p className="text-sm font-medium text-white">
                          {session.course}
          ...
          <div className="flex gap-2 pt-2">
            <button className="flex-1 py-2 bg-white text-black text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors">
              Schedule Session
            </button>
            <button
